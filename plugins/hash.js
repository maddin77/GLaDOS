...
GLaDOS.register({
    ...
    command('sha1', function (channel, user, name, text, params) {
        if (params.length === 0) {
            return user.notice('!sha1 <string>');
        }
    ...
    command('sha256', function (channel, user, name, text, params) {
        if (params.length === 0) {
            return user.notice('!sha256 <string>');
        }
    ...
    command('sha512', function (channel, user, name, text, params) {
        if (params.length === 0) {
            return user.notice('!sha512 <string>');
        }
    ...
    command('rmd160', function (channel, user, name, text, params) {
        if (params.length === 0) {
            return user.notice('!rmd160 <string>');
        }
    ...
    command('md5lookup', function (channel, user, name, text, params) {
        if (params.length === 0) {
            return user.notice('!md5lookup <md5 hash>');
        }
