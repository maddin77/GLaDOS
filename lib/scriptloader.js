        ...
        return {
            registerCommand: function (names, fn) {
                ...
                _.each(names, function (name) {
                    ...
                    connection.commands.on(name, fn);
                });
                return this;
            },
                    ...
                    if (_.has(SCRIPTS, scriptName)) {
                        ...
                        process.nextTick(function () {
                            SCRIPTS[scriptName].domain.run(function () {
                                SCRIPTS[scriptName].script = require(scriptFile)(_scriptLoader(scriptName), connection, glados);
                                debug('[%s] loaded.', scriptName);
                                if (!_.contains(glados.config.scripts, scriptName)) {
                                    glados.config.scripts.push(scriptName);
                                    glados.config.save();
                                }
                                if (fn) {
                                    fn(null);
    ...
    unloadScript = function (scriptName, fn) {
        ...
        if (cachedScript) {
            ...
            _.each(SCRIPTS[scriptName].commands, function (value, key) {
                ...
                connection.commands.removeListener(key, value);
            });
            ...
            _.each(SCRIPTS[scriptName].events, function (value, key) {
                ...
                connection.removeListener(key, value);
            });
            ...
            if (_.contains(glados.config.scripts, scriptName)) {
                glados.config.scripts = _.without(glados.config.scripts, scriptName);
                glados.config.save();
            }
            if (fn) {
                fn(null);
