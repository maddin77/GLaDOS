...
var logger = new (winston.Logger)({
    transports: [
        new (winston.transports.Console)({colorize: true, timestamp: true/*, prettyPrint: true*/})
    ]
});
...
if(!fs.existsSync(DATA_DIR + 'config.json')) {
    logger.info('config.json does not exist. Creating now.');
...
ircClient.addListener('topic', function(chanName, topic, nick, message) {
    ...
    logger.info('[%s] Topic: %s\nTopic set by %s. (%s)', channel.getName(), topic, nick, date.toString(), message);
...
ircClient.addListener('error', function(message) {
    logger.error('CLIENT ERROR: %s: %s', message.command, message.args.join(' '), message);
});
