...
if(!fs.existsSync('data/config.json')) {
    fs.createReadStream('data/default.config.json').pipe(fs.createWriteStream('data/config.json'));
}
            ...
            if(parts.length == 1) {
                ...
                plugins.forEach(function(plugin) {
                    if(plugin.name.toLowerCase() === parts[1].toLowerCase()) {
                        exist = true;
                        if(plugin.hasOwnProperty('desc')) {
                            user.say(plugin.desc);
                        }
...
ircClient.addListener('ctcp-version', function(from, to) {
    ircClient.ctcp(from, "notice", "VERSION " + config.get('version'));
});
