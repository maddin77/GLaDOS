...
Channel.prototype.userHasMode = function (nick, mode) {
    ...
    if (this.userModes.hasOwnProperty(nick)) {
        for (i = 0; i < this.userModes[nick].length; i += 1) {
            if (this.userModes[nick][i] === mode) {
                return true;
            }
        }
        return false;
...
Channel.prototype.userHasMinMode = function (nick, mode) {
    ...
    switch (mode) {
    case 'q':
        return this.userHasMode(nick, 'q');
    case 'a':
        return this.userHasMode(nick, 'q') || this.userHasMode(nick, 'a');
    case 'o':
        return this.userHasMode(nick, 'q') || this.userHasMode(nick, 'a') || this.userHasMode(nick, 'o');
    case 'h':
        return this.userHasMode(nick, 'q') || this.userHasMode(nick, 'a') || this.userHasMode(nick, 'o') || this.userHasMode(nick, 'h');
    case 'v':
        return this.userHasMode(nick, 'q') || this.userHasMode(nick, 'a') || this.userHasMode(nick, 'o') || this.userHasMode(nick, 'h') || this.userHasMode(nick, 'v');
    default:
        return false;
    }
};
...
Channel.prototype.setMode = function (mode) {
    var i;
    for (i = 0; i < this.modes.length; i += 1) {
