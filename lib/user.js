...
User.prototype.whois = function (callback) {
    ...
    this.client.whois(this.getNick(), function (info) {
        if (info.hasOwnProperty('user')) {
            that.setUserName(info.user);
        }
        if (info.hasOwnProperty('host')) {
            that.setHost(info.host);
        }
        if (info.hasOwnProperty('server')) {
            that.setServer(info.server);
        }
        if (info.hasOwnProperty('realname')) {
            that.setRealname(info.realname);
        }
        if (info.hasOwnProperty('account')) {
            that.setAccount(info.account);
        }
        if (info.hasOwnProperty('idle')) {
            that.setIdleTime(info.idle);
        }
        if (info.hasOwnProperty('channels')) {
            var chans = [];
            _.each(info.channels, function (value, key, list) {
                var mode = value.split('#')[0];
                if (mode !== '') {
                    that.server.getChannel('#' + value.split('#')[1]).addUserMode(info.nick, mode);
                }
                chans.push('#' + value.split('#')[1]);
            });
            that.setInChannels(chans);
        }
