...
module.exports = function (scriptLoader) {
    ...
    scriptLoader.on('command', 'isup', function (event) {
            ...
            if (host !== null) {
                request({
                    ...
                    if (!error && response.statusCode === 200) {
                        ...
                        scriptLoader.debug('[isup] %s', error);
                    }
                });
            } else {
    ...
    scriptLoader.on('command', 'geo', function (event) {
        if (event.params.length > 0) {
            request({
                ...
                if (!error && response.statusCode === 200) {
                    ...
                    event.channel.reply(event.user, 'Gratz. You broke it. (' + error + ')');
                    scriptLoader.debug('[geo] %s', error);
                }
            });
        } else {
    ...
    scriptLoader.on('command', 'dnslookup', function (event) {
        if (event.params.length > 0) {
            try {
                dns.lookup(event.text, function (error, address) {
                    if (error) {
                ...
                scriptLoader.debug('[dnslookup] %s', getError(error.code), error);
            }
        } else {
            event.user.notice('Use: !dnslookup <domain>');
        }
    });
    scriptLoader.on('command', 'dnsreverse', function (event) {
        if (event.params.length > 0) {
            try {
                ...
                event.channel.reply(event.user, 'Gratz. You broke it. (' + getError(error.code) + ')');
                scriptLoader.debug('[dnsreverse] %s', getError(error.code), error);
            }
        } else {
            event.user.notice('Use: !dnsreverse <ip>');
        }
    });
    scriptLoader.on('command', 'dnsresolve', function (event) {
        if (event.params.length > 0) {
            ...
            try {
                ...
                event.channel.reply(event.user, 'Gratz. You broke it. (' + getError(error.code) + ')');
                scriptLoader.debug('[dnsresolve] %s', getError(error.code), error);
            }
        } else {
            event.user.notice('Use: !dnsresolve <domain> [A/AAAA/MX/TXT/SRV/PTR/NS/CNAME]');
        }
    });
    scriptLoader.on('command', 'isip', function (event) {
        if (event.params.length > 0) {
